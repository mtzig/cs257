...
A command line program used to search for a list of books or authors in book1.csv based on a certain search string or search year.
...
def get_parsed_arguments():
  ...
  search_type_group.add_argument('-y', '--year', metavar='YEAR', nargs=2, type=int, help=year_help)
...
def main():
  ...
  #proccess arguments and displays output if user chose title search
  if arguments.title:
  ...
  #proccess arguments and displays output if user chose author search
  elif arguments.author:
    for author in data.authors(arguments.author[0]):
      print(author.given_name + ' ' + author.surname)
  
  #proccess arguments and displays output if user chose year search
  elif arguments.year:
